...
async function writeDefinitionToPublic(
  version: DefinitionVersion,
  definition: VIADefinitionV2 | VIADefinitionV3,
) {
  // evita chiamate in produzione
  if (!import.meta.env.DEV) return;

  const res = await fetch('/__via/dev/write-definition', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({
      version,
      viaDefinition: definition, // tu hai già la VIADefinition pronta
    }),
  });

  if (!res.ok) {
    const txt = await res.text().catch(() => '');
    throw new Error(`Write definition failed (${res.status}): ${txt}`);
  }
}
...
function importDefinitions(
  ...
  Promise.all(files.map(makeReaderPromise)).then((results) => {
    ...
    if (errors.length) {
      ...
      Promise.all(definitions.map((d) => writeDefinitionToPublic(version, d)))
    .then(() => {
      // 2) mantieni il tuo flusso attuale (UI immediata + custom defs)
    ...
    .catch((e: any) => {
      // se la scrittura fallisce, almeno lo mostri come errore
